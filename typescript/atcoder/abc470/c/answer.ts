...
//
// 元コードは クエリ 2 の全走査 + 毎回の reduce で N 回ずつ。それが Q 回で
// 2500 億回になり TLE。オーダーを落とす工夫は 2 つ。
//
// (A) 答えを作り直さず差分で直す
//     XOR は v ^ v = 0 なので、同じ値をもう一度 XOR すれば取り消せる。
//     `totalXor ^= 古い値 ^ 新しい値` で O(1) 更新。
//     古い値も要るのは、合計にすでに古い値が入っているから
//     （カゴの合計 300 円のりんごを取り替えるなら -100 してから +150）。
//
// (B) 1 以上の要素の添字だけ持つ
//     クエリ 2 は 0 の要素に何もしないので見に行くだけ無駄。
//     1 個処理する = 過去にクエリ 1 で 1 増やしている、なので
//     全クエリ 2 の処理回数は合計 Q 回まで（ならし計算量）。
